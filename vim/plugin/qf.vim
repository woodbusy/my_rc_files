...
call QF_addProgramAndFormat('grep', 'pcregrep --exclude \.*\.swp -n $*', "")
call QF_addProgramAndFormat('rgrep', 'pcregrep --exclude \.*\.swp --exclude_dir=\.svn -n -r $* /dev/null', "")
...
" added by EM - a project grep which is just a recursive grep in the current dir
call QF_addProgramAndFormat('pgrep', 'pcregrep --exclude \.*\.swp --exclude_dir=\.svn -n -r "$*" . /dev/null', "")
...
call QF_addProgramAndFormat('findgrep', 'find . \| pcregrep $*', "%f")
